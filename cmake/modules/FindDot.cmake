...
if( DOT_EXECUTABLE )
  ...
  if( DOT_EXECUTABLE )
    ...
    # LIBRARIES
    set( DOT_LIBRARIES )
    find_library( DOT_CDT_LIB NAMES cdt )
    if( NOT DOT_CDT_LIB )
      file( GLOB DOT_CDT_LIB /usr/lib/libcdt.so.? )  
    endif()
    if( WIN32 AND NOT DOT_CDT_LIB )
      BRAINVISA_FIND_FSENTRY( DOT_CDT_LIB PATTERNS "libcdt*" )
    endif()
    ...
    find_library( DOT_GVC_LIB NAMES gvc )
    if( NOT DOT_GVC_LIB )
      file( GLOB DOT_GVC_LIB /usr/lib/libgvc.so.? )
    endif()
    if( WIN32 AND NOT DOT_GVC_LIB )
      BRAINVISA_FIND_FSENTRY( DOT_GVC_LIB PATTERNS "libgvc*" )
    endif()
    if( DOT_GVC_LIB )
    ...
    find_library( DOT_PATHPLAN_LIB NAMES pathplan pathplan-4 )
    if( NOT DOT_PATHPLAN_LIB )
      file( GLOB DOT_PATHPLAN_LIB /usr/lib/libpathplan.so.? )
    endif()
    if( WIN32 AND NOT DOT_PATHPLAN_LIB )
      BRAINVISA_FIND_FSENTRY( DOT_PATHPLAN_LIB PATTERNS "libpathplan*" )
    endif()
    if( DOT_PATHPLAN_LIB )
    ...
    find_library( DOT_GRAPH_LIB NAMES graph graph-4 NO_CMAKE_ENVIRONMENT_PATH NO_CMAKE_PATH NO_CMAKE_SYSTEM_PATH )
    
    if( NOT DOT_GRAPH_LIB )
      file( GLOB DOT_GRAPH_LIB /usr/lib/libgraph.so.? )
    endif()
    if( WIN32 AND NOT DOT_GRAPH_LIB )
      BRAINVISA_FIND_FSENTRY( DOT_GRAPH_LIB PATTERNS "libpathplan*" )
    endif()
    if( DOT_GRAPH_LIB )
