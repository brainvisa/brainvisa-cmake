if( IS_DIRECTORY /i2bm )
  message( STATUS "Add specific paths for CEA - I2BM research labs" )
  execute_process( COMMAND systemIdentification RESULT_VARIABLE result OUTPUT_VARIABLE output OUTPUT_STRIP_TRAILING_WHITESPACE )
  if( result EQUAL 0 )
    list( FIND CMAKE_PREFIX_PATH "/i2bm/research/${output}" result )
    if( result EQUAL -1 )
      set( CMAKE_PREFIX_PATH ${CMAKE_PREFIX_PATH} "/i2bm/research/${output}" "/i2bm/research/${output}/vtkinria3d" )
      # Force to use VTK version installed in /i2bm/research by default
      file( GLOB VTK_DIR /i2bm/research/${output}/vtk/lib/vtk-* )
      if( VTK_DIR )
        set( VTK_DIR "${VTK_DIR}" CACHE PATH "The directory containing VTKConfig.cmake" )
      endif()
    endif()
  endif()
  foreach( soft dcmtk gsl )
    set( CMAKE_PREFIX_PATH ${CMAKE_PREFIX_PATH} "/i2bm/research/${soft}" )
  endforeach()
endif()