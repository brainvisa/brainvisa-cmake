...
@command_typer.command()
def update():
    src = command_context.soma_root / "src"
    ...
    with open(command_context.soma_root / "pixi.toml") as f:
        pixi = toml.load(f)
    dependencies = {
...
def packaging_plan(
    ...
    sources_conf_file = (
        command_context.soma_root / "conf" / "sources.d" / f"{environment_name}.json"
    )
    with open(sources_conf_file) as f:
        sources_conf = json.load(f)
    last_published_soma_env_version = sources_conf.get("latest_release")

    publication_file = command_context.soma_root / "conf" / "publication.json"
    ...
    component_sources = {}
    for json_file in (command_context.soma_root / "conf" / "sources.d").glob("*.json"):
    ...
    for recipe in sorted_recipies(command_context.soma_root):
        ...
        if components:
            ...
            recipe["build"]["script"] = "\n".join(
                (
                    f"cd '{command_context.soma_root}'",
                    f"pixi run --manifest-path='{command_context.soma_root}/pixi.toml' bash << END",
                    "set -x",
                    'cd "\\$SOMA_ROOT/build"',
    ...
    if release:
            ...
            actions.append(
                {
                    "action": "git_commit",
                    "kwargs": {
                        "repo": str(command_context.soma_root),
        ...
        packages_dir = command_context.soma_root / "plan" / "packages"
        actions.append(
            {
                "action": "publish",
                "kwargs": {
...
def graphviz(packages: str = "*", conda_forge=False, versions=False):
    ...
    all_soma_env_packages = set(find_soma_env_packages(command_context.soma_root))
